  ...
  const calcHormigon = () => {
   setError(''); setResH(null);
    ...
    const payload: DatosExportar = {
      ...
      dxfParams: {
        tipo: 'hormigon', subtipo: h.label,
        vol_m3: vol, cemento_bolsas: r.bolsas, cemento_kg: r.cementoKg,
        arena_m3: r.arenaM3, piedra_m3: r.piedraM3, agua_L: r.aguaL,
      },
    };
    setDatosH(payload);
...
const calcHierro = () => {
  setError(''); setResHierro(null);
   ...
   if (!pesoM || isNaN(lon) || lon <= 0 || isNaN(cant) || cant <= 0) { setError('Datos inválidos'); return; }
    ...
    const payload: DatosExportar = {
      ...
      dxfParams: {
        tipo: 'hierro', diam_mm: parseFloat(diam),
        kg_barra: rHierro.kgBarra, kg_total: rHierro.kgTotal,
        metros_totales: rHierro.metrosTotales,
      },
    };
  ...
  if (isNaN(luz) || luz <= 0 || isNaN(m2) || m2 <= 0) { setError('Valores inválidos'); return; }
   ...
   const espesor = Math.max(Math.round(luz / div * 100) / 100, 0.10);
    ...
    const payload: DatosExportar = {
      ...
      dxfParams: {
        tipo: 'losa', subtipo: losaTipo, m2, luz_m: luz,
        espesor_m: rLosa.espesor, hierro_kg_m2: rLosa.hierroKgM2,
        hormigon_total_m3: rLosa.hormigonTotal, hierro_total_kg: rLosa.hierroTotal,
      },
    };
   ...
   if (revTipo === 'grueso') {
     ...
     // Cal: 1 bolsa 5kg por cada bolsa cemento (mezcla 1:1:4 = cemento:cal:arena)
      ...
      const payload: DatosExportar = {
        ...
        dxfParams: {
          tipo: 'revoque', subtipo: 'grueso', m2,
          cemento_kg: rRev.cementoKg, bolsas: rRev.bolsas, arena_m3: rRev.arenaM3,
        },
      };
      setDatosRev(payload);
      publicarResultado(payload);
  ...
  const m2ConDesp = Math.round(m2 * (1 + desperdicio) * 10) / 10;
    ...
    const payload: DatosExportar = {
      ...
      dxfParams: {
        tipo: 'ceramico', subtipo: cerTam, m2_base: m2,
        m2_con_desp: rCer.cerConDesp, peg_bolsas: rCer.pegBolsas,
        pastina_kg: rCer.pastKg,
      },
    };
    setDatosCer(payload);
    publicarResultado(payload);
...
const calcContrapiso = () => {
  ...
  // Dato verificado: Hidralit rinde 2.6 m²/bolsa para contrapiso interior 10cm
    ...
    const payload: DatosExportar = {
      ...
      dxfParams: {
        tipo: 'contrapiso', m2, espesor_cm: parseFloat(cpEspesor),
        vol_m3: rCp.volM3, cemento_bolsas: rCp.cementoBolsas,
        arena_m3: rCp.arenaM3, cascote_m3: rCp.cascoteM3,
      },
    };
    setDatosCp(payload);
    publicarResultado(payload);
...
// qadm en kg/cm² → t/m²: 1 kg/cm² = 10 t/m²
    ...
    const payload: DatosExportar = {
      ...
      dxfParams: {
        tipo: 'zapata', carga_t: carga, qadm_kg_cm2: qadm,
        area_m2: rZap.area, lado_m: rZap.lado,
        espesor_m: rZap.espesorAprox, hormigon_m3: rZap.hormigonM3,
      },
    };
    setDatosZap(payload);
    publicarResultado(payload);
  ...
  const calcExcavacion = () => {
   ...
   const l = parseFloat(excLargo); const a = parseFloat(excAncho); const p = parseFloat(excProf);
    ...
    const payload: DatosExportar = {
      ...
      dxfParams: {
        tipo: 'excavacion', tipo_suelo: tipoSuelo,
        largo_m: l, ancho_m: a, prof_m: p,
        vol_natural_m3: rExc.volNatural, vol_esponjado_m3: rExc.volEsponjado,
        camiones: rExc.camiones, esponj_pct: rExc.esponj,
      },
    };
    setDatosExc(payload);
    publicarResultado(payload);
...
const calcMortero = () => {
   ...
   const props: Record<string, { c: number; a: number }> = {
    ...
    const payload: DatosExportar = {
      ...
      dxfParams: {
        tipo: 'mortero', proporcion: mortProp, vol_m3: vol,
        cemento_bolsas: rMort.cementoBolsas, cemento_kg: rMort.cementoKg,
        arena_m3: rMort.arenaM3,
      },
    };
  ...
  const calcRendimiento = () => {
   ...
   const cant = parseFloat(rendCant); const cuad = parseInt(rendCuad);
    ...
    const payload: DatosExportar = {
      ...
      dxfParams: {
        tipo: 'rendimiento', tarea: r.desc, unidad: r.unidad,
        cantidad: cant, cuadrilla: cuad,
        horas: rRend.horas, dias: rRend.dias,
      },
    };
