...
// MÓDULO ELECTRICIDAD INDUSTRIAL — INGENIUM PRO v8.0
// Verificado: IEC 60228, NEC 2023 §310.15(B)(16), IEC 60364-5-52,
// IEC 60909, API RP 500:2012, IEC 60079-10-1, IEC 60079-0,
// IEEE 80, IES/IESNA, EN 12464-1, API RP 54 §5.9, IEC 60076, IEEE C57
...
const [trFP, setTrfp] = useState('0.85');
  ...
  const calcCable = () => {
   ...
   if (isNaN(I)||I<=0){setErr('Corriente inválida');return;}
    const entry = Object.entries(CABLES).find(([,c])=>(cMat==='cu'?c.Iz_cu:c.Iz_al)>=I);
    if(!entry){setErr('Corriente excede catálogo — consultar ingeniería especializada');return;}
...
const calcCaida = () => {
  ...
  const L=parseFloat(cdL),I=parseFloat(cdI),V=parseFloat(cdV),FP=parseFloat(cdFP);
   if([L,I,V,FP].some(n=>isNaN(n)||n<=0)){setErr('Valores inválidos');return;}
  ...
  const calcCC = () => {
   ...
   if([S,uk,V,L].some(n=>isNaN(n)||n<=0)){setErr('Valores inválidos');return;}
...
const tan1=Math.tan(Math.acos(FP1)), tan2=Math.tan(Math.acos(FP2));
  ...
  const P=parseFloat(mP),V=parseFloat(mV),FP=parseFloat(mFP),eta=parseFloat(mEta);
    if([P,V,FP,eta].some(n=>isNaN(n)||n<=0)){setErr('Valores inválidos');return;}
const Pelec=P/eta;
  ...
  const calcLuz = () => {
    ...
    if([A,Phi,eta,MF].some(n=>isNaN(n)||n<=0)){setErr('Valores inválidos');return;}
...
const calcTrafo = () => {
    ...
    if([P,FP,eta,Vp,Vs].some(n=>isNaN(n)||n<=0)){setErr('Valores inválidos');return;}
    const S=P/(FP*eta);
      ...
      {sub==='cable'&&(
        <div>
          ...
          <div style={g3}>
            ...
            <div><label style={lbl}>Corriente de diseño (A)</label>
       ...
       <div>
         ...
         <div style={g2}>
          ...
          <div><label style={lbl}>Corriente (A)</label>
            ...
            <div><label style={lbl}>Tensión nominal (V)</label>
           ...
           <div><label style={lbl}>Factor de potencia (cos φ)</label>
     ...
     {sub==='cc'&&(
        <div>
         ...
         <div><label style={lbl}>Potencia transformador (kVA)</label>
           ...
           <div><label style={lbl}>Tensión secundaria (V)</label>
   ...
   {sub==='fp'&&(
      ...
      {sub==='motor'&&(
        <div>
          ...
          <div style={g2}>
            <div><label style={lbl}>Potencia mecánica en eje (kW)</label>
              <input value={mP} onChange={e=>setMp(e.target.value)} style={inp} type="number" min="0.1" step="0.5"/>
            </div>
            <div><label style={lbl}>Tensión de alimentación (V)</label>
            ...
            <div><label style={lbl}>Factor de potencia motor (cos φ)</label>
            ...
            <div><label style={lbl}>Rendimiento motor (η)</label>
      ...
      {sub==='peligrosa'&&(
        <div>
          ...
          <div style={g3}>
            <div><label style={lbl}>Industria / Aplicación</label>
              <select value={apInd} onChange={e=>setApind(e.target.value)} style={inp}>
                {['Petróleo / Gas','Perforación petróleo','Minería carbón (grisú)','Química / Petroquímica','Represas (H₂ baterías)','MMO — Pintura / Solventes','Gas natural licuado (GNL)'].map(i=><option key={i} value={i} style={{background:'#0a0f1e'}}>{i}</option>)}
              </select>
            </div>
            <div><label style={lbl}>Gas / Polvo / Vapores presentes</label>
              <select value={apGas} onChange={e=>setApgas(e.target.value)} style={inp}>
                {['Metano (CH₄) — Grupo IIA','Propano / Butano — Grupo IIA','Etileno — Grupo IIB','Hidrógeno (H₂) — Grupo IIC','Acetileno — Grupo IIC','Grisú (minas carbón) — Grupo I','Polvo carbón — Zona 21','Gasolina / Nafta — Grupo IIA'].map(g=><option key={g} value={g} style={{background:'#0a0f1e'}}>{g}</option>)}
              </select>
            </div>
            <div><label style={lbl}>Zona según frecuencia de exposición</label>
          ...
          <div style={{background:`${zd.color}15`,border:`1px solid ${zd.color}40`,borderRadius:16,padding:20,marginBottom:16}}>
            ...
            <div style={g2}>
              <div style={{background:'#0a0f1e',borderRadius:10,padding:12}}>
                <div style={{fontSize:9,color:'#475569',textTransform:'uppercase' as const,marginBottom:4}}>Descripción IEC 60079-10-1</div>
      ...
      {sub==='luz'&&(
        <div>
          ...
          <div style={g2}>
            <div><label style={lbl}>Tipo de recinto / Aplicación</label>
              <select value={lApp} onChange={e=>setLapp(e.target.value)} style={inp}>
                {Object.entries(LUX_APP).map(([k,v])=><option key={k} value={k} style={{background:'#0a0f1e'}}>{v.sector} — {v.desc} ({v.lux} lux) · {v.norma}</option>)}
              </select>
            </div>
            <div><label style={lbl}>Área a iluminar (m²)</label>
            ...
            <div><label style={lbl}>Eficiencia luminaria (η)</label>
      ...
      {sub==='trafo'&&(
        <div>
          ...
          <div style={g2}>
            ...
            <div><label style={lbl}>Eficiencia transformador (η)</label>
            ...
            <div><label style={lbl}>Tensión primaria (kV)</label>
