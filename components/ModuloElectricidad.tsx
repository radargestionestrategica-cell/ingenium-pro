...
// No se modifican fórmulas ni estructura visual del módulo.
...
const CABLES: Record<string, { R_cu: number; R_al: number; Iz_cu: number; Iz_al: number }> = {
  ...
  '2.5 mm²':  { R_cu: 7.41,  R_al: 12.10, Iz_cu: 20,  Iz_al: 15  },
  '4 mm²':    { R_cu: 4.61,  R_al: 7.41,  Iz_cu: 30,  Iz_al: 25  },
  '6 mm²':    { R_cu: 3.08,  R_al: 4.61,  Iz_cu: 50,  Iz_al: 40  },
  '10 mm²':   { R_cu: 1.83,  R_al: 3.08,  Iz_cu: 65,  Iz_al: 50  },
  '16 mm²':   { R_cu: 1.15,  R_al: 1.91,  Iz_cu: 85,  Iz_al: 65  },
  '25 mm²':   { R_cu: 0.727, R_al: 1.20,  Iz_cu: 100, Iz_al: 80  },
  '35 mm²':   { R_cu: 0.524, R_al: 0.868, Iz_cu: 115, Iz_al: 95  },
  '50 mm²':   { R_cu: 0.387, R_al: 0.641, Iz_cu: 130, Iz_al: 105 },
  '70 mm²':   { R_cu: 0.268, R_al: 0.443, Iz_cu: 150, Iz_al: 125 },
  '95 mm²':   { R_cu: 0.193, R_al: 0.320, Iz_cu: 175, Iz_al: 150 },
  '120 mm²':  { R_cu: 0.153, R_al: 0.253, Iz_cu: 200, Iz_al: 165 },
  '150 mm²':  { R_cu: 0.124, R_al: 0.206, Iz_cu: 230, Iz_al: 190 },
  '185 mm²':  { R_cu: 0.0991,R_al: 0.164, Iz_cu: 255, Iz_al: 210 },
  '240 mm²':  { R_cu: 0.0754,R_al: 0.125, Iz_cu: 310, Iz_al: 260 },
};
...
const SUBS: { id: Sub; label: string; icon: string }[] = [
  ...
  { id:'caida',     label:'Caída Tensión',    icon:'⚡' },
  { id:'cc',        label:'Cortocircuito',    icon:'💥' },
  { id:'fp',        label:'Factor Potencia',  icon:'🔋' },
...
function Tit({ t }: { t: string }) {
  return (
    <div style={{ fontSize:11, color:COLOR, fontWeight:700, letterSpacing:1, marginBottom:16, textTransform:'uppercase' }}>
      {t}
    </div>
  );
}
...
function Info({ t }: { t: string }) {
  return (
    <div style={{ fontSize:12, color:'#475569', marginBottom:12, padding:'8px 12px', background:'rgba(34,197,94,0.05)', borderRadius:8 }}>
      {t}
    </div>
  );
}

function Warn({ t }: { t: string }) {
  return (
    <div style={{ fontSize:11, color:'#f59e0b', padding:'8px 12px', background:'rgba(245,158,11,0.08)', border:'1px solid rgba(245,158,11,0.2)', borderRadius:8, marginTop:8 }}>
      {t}
    </div>
  );
}
...
function Err({ t }: { t: string }) {
  return (
    <div style={{ padding:'10px 16px', borderRadius:10, marginBottom:16, background:'rgba(239,68,68,0.08)', border:'1px solid rgba(239,68,68,0.3)', color:'#f87171', fontSize:13 }}>
      {t}
    </div>
  );
}
...
function Card({ label, val, sub, color }: { label: string; val: string; sub?: string; color?: string }) {
  return (
    <div style={{ background:'#0a0f1e', borderRadius:10, padding:14 }}>
      <div style={{ fontSize:9, color:'#475569', textTransform:'uppercase', marginBottom:4, letterSpacing:0.4 }}>{label}</div>
...
function ResBox({ children, ok }: { children: ReactNode; ok?: boolean }) {
  ...
  return (
    <div style={{ background:bg, border:`1px solid ${br}`, borderRadius:16, padding:20 }}>
      {children}
    </div>
  );
}
...
export default function ModuloElectricidad() {
  ...
  // CAÍDA STATE
  const [cdCable, setCdcable] = useState('25 mm²');
  ...
  // ÁREA PELIGROSA STATE
  const [apZona, setApzona] = useState<'zona0'|'zona1'|'zona2'>('zona1');
  const [apGas, setApgas] = useState('Metano (CH₄) — Grupo IIA');
  const [apInd, setApind] = useState('Petróleo / Gas');
  ...
  const calcCable = () => {
    ...
    if (!entry) {
      setErr('Corriente excede catálogo — consultar ingeniería especializada');
      return;
    }

    const resultadoCable = {
      calibre: entry[0],
      Iz: cMat === 'cu' ? entry[1].Iz_cu : entry[1].Iz_al,
      I,
    };
  ...
  const calcCaida = () => {
    ...
    const L = parseFloat(cdL);
    const I = parseFloat(cdI);
    const V = parseFloat(cdV);
    const FP = parseFloat(cdFP);

    if ([L, I, V, FP].some((n) => isNaN(n) || n <= 0)) {
    ...
    if (!c) {
      setErr('Cable no encontrado');
      return;
    }

    // R a 75°C: R_75 = R_20 × (1 + 0.00393 × 55) = R_20 × 1.216 — IEC 60228
    const R75 = c.R_cu * 1.216 / 1000; // Ω/m
    const X = 0.00008; // Ω/m reactancia típica para cable en conduit
  ...
  const calcCC = () => {
    ...
    const S = parseFloat(ccS);
    const uk = parseFloat(ccUk);
    const V = parseFloat(ccV);
    const L = parseFloat(ccL);

    if ([S, uk, V, L].some((n) => isNaN(n) || n <= 0)) {
      setErr('Valores inválidos');
      return;
    }
  ...
  const calcFP = () => {
    ...
    const P = parseFloat(fpP);
    const FP1 = parseFloat(fpFP1);
    const FP2 = parseFloat(fpFP2);
    const V = parseFloat(fpV);
    const f = parseFloat(fpF);

    if ([P, FP1, FP2, V, f].some((n) => isNaN(n) || n <= 0) || FP1 >= FP2) {
      setErr('FP2 debe ser mayor que FP1');
      return;
    }

    const tan1 = Math.tan(Math.acos(FP1));
    const tan2 = Math.tan(Math.acos(FP2));
    const Qc = P * (tan1 - tan2);

    // C = Qc×1000 / (2π × f × V²) — en µF por fase
  ...
  const calcMotor = () => {
    ...
    const P = parseFloat(mP);
    const V = parseFloat(mV);
    const FP = parseFloat(mFP);
    const eta = parseFloat(mEta);

    if ([P, V, FP, eta].some((n) => isNaN(n) || n <= 0)) {
      setErr('Valores inválidos');
      return;
    }
    ...
    const Iarr = 6 * Inom; // DOL típico × 6

    // Cable: 1.25 × I_nom — NEC 430.52
    const Idis = 1.25 * Inom;
    const ce = Object.entries(CABLES).find(([, c]) => c.Iz_cu >= Idis);

    const resultadoMotor = {
      Inom: Math.round(Inom * 10) / 10,
      Iarr: Math.round(Iarr),
      Pelec: Math.round(Pelec * 10) / 10,
      calibre: ce ? ce[0] : 'Consultar especialista',
    };
  ...
  const calcLuz = () => {
    ...
    const A = parseFloat(lArea);
    const Phi = parseFloat(lLm);
    const eta = parseFloat(lEta);
    const MF = parseFloat(lMF);

    if ([A, Phi, eta, MF].some((n) => isNaN(n) || n <= 0)) {
      setErr('Valores inválidos');
      return;
    }

    const app = LUX_APP[lApp];
    const CU = 0.60; // Coeficiente de utilización estándar IES
  ...
  const calcTrafo = () => {
    ...
    const P = parseFloat(trP);
    const FP = parseFloat(trFP);
    const eta = parseFloat(trEta);
    const Vp = parseFloat(trVp);
    const Vs = parseFloat(trVs);

    if ([P, FP, eta, Vp, Vs].some((n) => isNaN(n) || n <= 0)) {
      setErr('Valores inválidos');
      return;
    }
  ...
  const publicarAreaPeligrosa = () => {
    ...
    const zona = ZONAS[apZona];

    const resultadoArea = {
      industria: apInd,
      gasPolvoVapor: apGas,
      zonaIEC: zona.iec,
      divisionAPI: zona.api,
  ...
  return (
    <div style={{ padding:24, color:'#f1f5f9', fontFamily:'Inter,sans-serif', maxWidth:960, margin:'0 auto' }}>
      ...
      <div style={{ background:'linear-gradient(135deg,rgba(34,197,94,0.15),rgba(34,197,94,0.05))', border:'1px solid rgba(34,197,94,0.3)', borderRadius:16, padding:24, marginBottom:16, display:'flex', alignItems:'center', gap:16 }}>
        <div style={{ width:52, height:52, borderRadius:14, background:'linear-gradient(135deg,#22c55e,#16a34a)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:26, flexShrink:0 }}>⚡</div>
        <div style={{ flex:1 }}>
      ...
      {/* TABS */}
      <div style={{ display:'flex', background:'#0a0f1e', borderRadius:12, padding:4, marginBottom:24, border:'1px solid rgba(34,197,94,0.15)', overflowX:'auto', gap:3 }}>
        {SUBS.map((s) => (
          <button
            key={s.id}
            onClick={() => { setSub(s.id); R(); }}
            style={{
              flex:1,
              padding:'9px 8px',
              border:'none',
              ...
              fontWeight:700,
              whiteSpace:'nowrap',
              background:sub === s.id ? 'linear-gradient(135deg,#22c55e,#16a34a)' : 'transparent',
              color:sub === s.id ? '#fff' : '#475569',
              boxShadow:sub === s.id ? '0 4px 12px rgba(34,197,94,0.4)' : 'none',
            }}
          >
            {s.icon} {s.label}
          </button>
      ...
      {sub === 'cable' && (
        <div>
          ...
          <div style={g3}>
            <div>
              <label style={lbl}>Tipo circuito</label>
              <select value={cTipo} onChange={(e) => setCt(e.target.value as 'monof'|'trif')} style={inp}>
                <option value="monof" style={{ background:'#0a0f1e' }}>Monofásico 1F</option>
                <option value="trif" style={{ background:'#0a0f1e' }}>Trifásico 3F</option>
              </select>
            </div>

            <div>
              <label style={lbl}>Corriente de diseño (A)</label>
              <input value={cI} onChange={(e) => setCi(e.target.value)} style={inp} type="number" min="1" step="1" />
            </div>

            <div>
              <label style={lbl}>Material conductor</label>
              <select value={cMat} onChange={(e) => setCmat(e.target.value as 'cu'|'al')} style={inp}>
                <option value="cu" style={{ background:'#0a0f1e' }}>Cobre (Cu) — Recomendado</option>
                <option value="al" style={{ background:'#0a0f1e' }}>Aluminio (Al) — ≥16 mm²</option>
              </select>
            </div>
          </div>
          ...
          {resCable && (
            <ResBox>
              <div style={{ fontSize:12, color:COLOR, fontWeight:700, marginBottom:14 }}>✅ CALIBRE RECOMENDADO</div>

              <div style={g3}>
                <Card label="Calibre mínimo" val={resCable.calibre} />
                <Card label="Capacidad Iz" val={`${resCable.Iz} A`} sub="A 75°C en conduit" />
                <Card label="Corriente diseño" val={`${resCable.I} A`} />
              </div>

              <div style={{ background:'#0a0f1e', borderRadius:10, padding:14, marginTop:12 }}>
                <div style={{ fontSize:10, color:COLOR, fontWeight:700, marginBottom:8 }}>TABLA COMPLETA IEC 60228 / NEC — COBRE 75°C EN CONDUIT</div>

                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:2 }}>
                  <div style={{ fontSize:9, color:'#334155', fontWeight:700 }}>CALIBRE</div>
                  <div style={{ fontSize:9, color:'#334155', fontWeight:700 }}>R (Ω/km) 20°C</div>
      ...
      {sub === 'caida' && (
        <div>
          ...
          <div style={g2}>
            <div>
              <label style={lbl}>Cable seleccionado</label>
              <select value={cdCable} onChange={(e) => setCdcable(e.target.value)} style={inp}>
                {Object.keys(CABLES).map((k) => (
                  <option key={k} value={k} style={{ background:'#0a0f1e' }}>{k}</option>
                ))}
              </select>
            </div>
            ...
            <div>
              ...
              <select value={cdTipo} onChange={(e) => setCdtipo(e.target.value as 'monof'|'trif')} style={inp}>
                <option value="monof" style={{ background:'#0a0f1e' }}>Monofásico (2×L)</option>
                <option value="trif" style={{ background:'#0a0f1e' }}>Trifásico (√3×L)</option>
              </select>
            </div>

            <div>
              <label style={lbl}>Longitud del cable (m)</label>
              <input value={cdL} onChange={(e) => setCdl(e.target.value)} style={inp} type="number" min="1" step="1" />
            </div>

            <div>
              <label style={lbl}>Corriente (A)</label>
              <input value={cdI} onChange={(e) => setCdi(e.target.value)} style={inp} type="number" min="1" step="1" />
            </div>

            <div>
              <label style={lbl}>Tensión nominal (V)</label>
              <select value={cdV} onChange={(e) => setCdv(e.target.value)} style={inp}>
                {['127','220','380','400','440','660','1000','6600','13200'].map((v) => (
                  <option key={v} value={v} style={{ background:'#0a0f1e' }}>{v} V</option>
                ))}
              </select>
            </div>

            <div>
              <label style={lbl}>Factor de potencia (cos φ)</label>
              <input value={cdFP} onChange={(e) => setCdfp(e.target.value)} style={inp} type="number" min="0.1" max="1" step="0.01" />
            </div>
          </div>

          <Info t="Trifásico: ΔV = √3 × L × I × (R×cosφ + X×sinφ) | R a 75°C: R_20 × 1.216 | Límite: 4% IEC · 3% NEC 210.19" />

          <Btn onClick={calcCaida} text="Calcular caída de tensión" />

          {resCd && (
            <ResBox ok={resCd.ok}>
              <div style={{ fontSize:12, color:resCd.ok ? COLOR : '#f87171', fontWeight:700, marginBottom:14 }}>
                {resCd.ok ? '✅ DENTRO DEL LÍMITE — IEC/NEC' : '❌ EXCEDE LÍMITE — Aumentar calibre o reducir longitud'}
              </div>

              <div style={g3}>
                <Card label="Caída de tensión" val={`${resCd.dV} V`} />
                <Card label="Caída %" val={`${resCd.pct}%`} color={resCd.ok ? COLOR : '#ef4444'} />
                <Card label="Límite máximo" val="4% IEC · 3% NEC" sub="IEC 60364 / NEC 210.19" />
              </div>
            </ResBox>
          )}
        </div>
      ...
      {sub === 'cc' && (
        <div>
          ...
          <div style={g2}>
            <div>
              <label style={lbl}>Potencia transformador (kVA)</label>
              <input value={ccS} onChange={(e) => setCcs(e.target.value)} style={inp} type="number" min="1" step="25" />
            </div>

            <div>
              <label style={lbl}>Impedancia de cc uk (%)</label>
              <input value={ccUk} onChange={(e) => setCcuk(e.target.value)} style={inp} type="number" min="1" max="20" step="0.5" />
              <div style={{ fontSize:10, color:'#334155', marginTop:4 }}>Distribución: 4-6% · Alta tensión: 8-12%</div>
            </div>

            <div>
              <label style={lbl}>Tensión secundaria (V)</label>
              <select value={ccV} onChange={(e) => setCcv(e.target.value)} style={inp}>
                {['220','380','400','440','660'].map((v) => (
                  <option key={v} value={v} style={{ background:'#0a0f1e' }}>{v} V</option>
                ))}
              </select>
            </div>
            ...
            <div>
              <label style={lbl}>Cable al tablero principal</label>
          ...
          <div style={g2}>
            ...
            <div>
              ...
              <select value={fpF} onChange={(e) => setFpf(e.target.value)} style={inp}>
                <option value="50" style={{ background:'#0a0f1e' }}>50 Hz — Argentina, Europa, África</option>
                <option value="60" style={{ background:'#0a0f1e' }}>60 Hz — EE.UU., México, Colombia, Perú</option>
              </select>
            </div>
          </div>

          <Info t="Qc = P × (tan φ1 − tan φ2) kVAR | C = Qc×1000 / (2π × f × V²) µF por fase" />

          <Btn onClick={calcFP} text="Calcular banco de capacitores" />

          {resFP && (
            <ResBox>
              <div style={{ fontSize:12, color:COLOR, fontWeight:700, marginBottom:14 }}>RESULTADO — BANCO DE CAPACITORES</div>

              <div style={g4}>
                <Card label="Potencia reactiva Qc" val={`${resFP.Qc} kVAR`} />
                <Card label="Capacidad por fase" val={`${resFP.C} µF`} />
                <Card label="Corriente antes" val={`${resFP.I1} A`} color="#ef4444" />
                <Card label="Corriente después" val={`${resFP.I2} A`} color={COLOR} />
              </div>
      ...
      {sub === 'motor' && (
        <div>
          <Tit t="Motor eléctrico trifásico — NEC Art. 430 / IEC 60947 / NEMA MG1" />

          <div style={g2}>
            <div>
              <label style={lbl}>Potencia mecánica en eje (kW)</label>
              <input value={mP} onChange={(e) => setMp(e.target.value)} style={inp} type="number" min="0.1" step="0.5" />
            </div>

            <div>
              <label style={lbl}>Tensión de alimentación (V)</label>
              <select value={mV} onChange={(e) => setMv(e.target.value)} style={inp}>
                {['220','380','400','440','660','2300','4160'].map((v) => (
                  <option key={v} value={v} style={{ background:'#0a0f1e' }}>{v} V</option>
                ))}
              </select>
            </div>
            ...
            <div>
              <label style={lbl}>Factor de potencia motor (cos φ)</label>
              <input value={mFP} onChange={(e) => setMfp(e.target.value)} style={inp} type="number" min="0.5" max="1" step="0.01" />
              <div style={{ fontSize:10, color:'#334155', marginTop:4 }}>Motor industrial típico: 0.82-0.90</div>
            </div>

            <div>
              <label style={lbl}>Rendimiento motor (η)</label>
              <input value={mEta} onChange={(e) => setMeta(e.target.value)} style={inp} type="number" min="0.5" max="1" step="0.01" />
              <div style={{ fontSize:10, color:'#334155', marginTop:4 }}>IE2: 0.90-0.95 · IE3: 0.93-0.97</div>
            </div>
          </div>

          <Info t="I_nom = P_elec / (√3 × V × FP) | I_arranque DOL ≈ 6 × I_nom | Cable: 1.25 × I_nom (NEC 430.52)" />

          <Btn onClick={calcMotor} text="Calcular motor y cable" />

          {resMot && (
            <ResBox>
              <div style={{ fontSize:12, color:COLOR, fontWeight:700, marginBottom:14 }}>RESULTADO — MOTOR ELÉCTRICO TRIFÁSICO</div>

              <div style={g4}>
                <Card label="I nominal" val={`${resMot.Inom} A`} />
                <Card label="I arranque DOL" val={`${resMot.Iarr} A`} color="#f59e0b" sub="≈6 × I_nom — NEC 430" />
                <Card label="P eléctrica" val={`${resMot.Pelec} kW`} sub="Consumo real red" />
      ...
      {sub === 'peligrosa' && (
        <div>
          ...
          <div style={g3}>
            <div>
              <label style={lbl}>Industria / Aplicación</label>
              <select value={apInd} onChange={(e) => setApind(e.target.value)} style={inp}>
                {['Petróleo / Gas','Perforación petróleo','Minería carbón (grisú)','Química / Petroquímica','Represas (H₂ baterías)','MMO — Pintura / Solventes','Gas natural licuado (GNL)'].map((i) => (
                  <option key={i} value={i} style={{ background:'#0a0f1e' }}>{i}</option>
                ))}
              </select>
            </div>

            <div>
              <label style={lbl}>Gas / Polvo / Vapores presentes</label>
              <select value={apGas} onChange={(e) => setApgas(e.target.value)} style={inp}>
                {['Metano (CH₄) — Grupo IIA','Propano / Butano — Grupo IIA','Etileno — Grupo IIB','Hidrógeno (H₂) — Grupo IIC','Acetileno — Grupo IIC','Grisú (minas carbón) — Grupo I','Polvo carbón — Zona 21','Gasolina / Nafta — Grupo IIA'].map((g) => (
                  <option key={g} value={g} style={{ background:'#0a0f1e' }}>{g}</option>
                ))}
              </select>
            </div>

            <div>
              <label style={lbl}>Zona según frecuencia de exposición</label>
              <select value={apZona} onChange={(e) => setApzona(e.target.value as 'zona0'|'zona1'|'zona2')} style={inp}>
                <option value="zona0" style={{ background:'#0a0f1e' }}>Zona 0 / Div.1 — CONTINUA</option>
                <option value="zona1" style={{ background:'#0a0f1e' }}>Zona 1 / Div.1 — OCASIONAL</option>
                <option value="zona2" style={{ background:'#0a0f1e' }}>Zona 2 / Div.2 — INFRECUENTE</option>
              </select>
            </div>
          </div>

          <Btn onClick={publicarAreaPeligrosa} text="Generar clasificación exportable" />
          ...
          <div style={{ background:`${zd.color}15`, border:`1px solid ${zd.color}40`, borderRadius:16, padding:20, marginBottom:16 }}>
            ...
            <div style={g2}>
              <div style={{ background:'#0a0f1e', borderRadius:10, padding:12 }}>
                <div style={{ fontSize:9, color:'#475569', textTransform:'uppercase', marginBottom:4 }}>Descripción IEC 60079-10-1</div>
                <div style={{ fontSize:12, color:'#f1f5f9' }}>{zd.desc}</div>
              </div>

              <div style={{ background:'#0a0f1e', borderRadius:10, padding:12 }}>
                <div style={{ fontSize:9, color:'#475569', textTransform:'uppercase', marginBottom:4 }}>Equipo Ex requerido</div>
          ...
          <div style={{ background:'rgba(34,197,94,0.05)', border:'1px solid rgba(34,197,94,0.15)', borderRadius:12, padding:16 }}>
            <div style={{ fontSize:11, color:COLOR, fontWeight:700, marginBottom:10 }}>GRUPOS IEC 60079-0 — TEMPERATURA DE IGNICIÓN</div>

            {[
              { g:'Grupo I',   d:'Grisú — minas subterráneas carbón', T:'—',     ej:'Minería subterránea' },
              { g:'Grupo IIA', d:'Propano, benceno, metano, butano',  T:'T1-T3', ej:'Petróleo, petroquímica' },
              { g:'Grupo IIB', d:'Etileno, gas de ciudad',            T:'T1-T4', ej:'Plantas de etileno' },
              { g:'Grupo IIC', d:'H₂ (hidrógeno), acetileno',         T:'T1-T6', ej:'Baterías, GNL, H₂' },
            ].map((r, i) => (
      ...
      {sub === 'luz' && (
        <div>
          <Tit t="Diseño de iluminación industrial — IES/IESNA · EN 12464-1 · API RP 54 · ISO 8995" />

          <div style={g2}>
            <div>
              <label style={lbl}>Tipo de recinto / Aplicación</label>
              <select value={lApp} onChange={(e) => setLapp(e.target.value)} style={inp}>
                {Object.entries(LUX_APP).map(([k, v]) => (
                  <option key={k} value={k} style={{ background:'#0a0f1e' }}>
                    {v.sector} — ({v.lux} lux) · {v.norma}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div>
              <label style={lbl}>Área a iluminar (m²)</label>
              <input value={lArea} onChange={(e) => setLarea(e.target.value)} style={inp} type="number" min="1" step="1" />
            </div>

            <div>
              <label style={lbl}>Flujo luminoso por luminaria (lm)</label>
              <input value={lLm} onChange={(e) => setLlm(e.target.value)} style={inp} type="number" min="100" step="100" />
              <div style={{ fontSize:10, color:'#334155', marginTop:4 }}>LED 50W≈5000lm · LED 100W≈10000lm · HPS 250W≈27000lm</div>
            </div>

            <div>
              <label style={lbl}>Eficiencia luminaria (η)</label>
              <input value={lEta} onChange={(e) => setLeta(e.target.value)} style={inp} type="number" min="0.3" max="1" step="0.05" />
              <div style={{ fontSize:10, color:'#334155', marginTop:4 }}>LED moderno: 0.85 · Convencional: 0.65</div>
            </div>

            <div>
              <label style={lbl}>Factor de mantenimiento (MF)</label>
              <input value={lMF} onChange={(e) => setLmf(e.target.value)} style={inp} type="number" min="0.5" max="1" step="0.05" />
              <div style={{ fontSize:10, color:'#334155', marginTop:4 }}>Limpio: 0.80 · Industrial: 0.65 · Minería: 0.60</div>
            </div>
          </div>

          <Info t={`Iluminancia requerida: ${LUX_APP[lApp].lux} lux — ${LUX_APP[lApp].norma} | N = (E × A) / (Φ × η × MF × CU) | CU=0.60`} />

          <Btn onClick={calcLuz} text="Calcular número de luminarias" />

          {resLuz && (
            <ResBox ok={resLuz.ok}>
              <div style={{ fontSize:12, color:resLuz.ok ? COLOR : '#f87171', fontWeight:700, marginBottom:14 }}>
                {resLuz.ok ? '✅ DISEÑO CUMPLE NORMATIVA' : '❌ INSUFICIENTE — Agregar luminarias o aumentar potencia'}
              </div>
              ...
              <div style={g3}>
                ...
                <Card label="Requerimiento normativo" val={`${LUX_APP[lApp].lux} lux mín.`} sub={LUX_APP[lApp].norma} />
              </div>
            </ResBox>
          )}
        </div>
      ...
      {sub === 'trafo' && (
        <div>
          <Tit t="Dimensionamiento de transformador — IEC 60076 / IEEE C57 / CIRSOC" />

          <div style={g2}>
            <div>
              <label style={lbl}>Potencia total de carga (kW)</label>
              <input value={trP} onChange={(e) => setTrp(e.target.value)} style={inp} type="number" min="1" step="10" />
            </div>

            <div>
              <label style={lbl}>Factor de potencia de la carga</label>
              <input value={trFP} onChange={(e) => setTrfp(e.target.value)} style={inp} type="number" min="0.5" max="1" step="0.01" />
            </div>

            <div>
              <label style={lbl}>Eficiencia transformador (η)</label>
              <input value={trEta} onChange={(e) => setTreta(e.target.value)} style={inp} type="number" min="0.85" max="1" step="0.005" />
              <div style={{ fontSize:10, color:'#334155', marginTop:4 }}>Distribución: 0.97-0.99 (IEC 60076)</div>
            </div>

            <div>
              <label style={lbl}>Tensión primaria (kV)</label>
              <select value={trVp} onChange={(e) => setTrvp(e.target.value)} style={inp}>
                {['2300','4160','6600','13200','22000','33000','66000','132000'].map((v) => (
                  <option key={v} value={v} style={{ background:'#0a0f1e' }}>
                    {(parseInt(v) / 1000).toFixed(1)} kV
                  </option>
                ))}
              </select>
            </div>
            ...
            <div>
              ...
              <select value={trVs} onChange={(e) => setTrvs(e.target.value)} style={inp}>
                {['220','380','400','440','660','1000'].map((v) => (
                  <option key={v} value={v} style={{ background:'#0a0f1e' }}>{v} V</option>
                ))}
              </select>
            </div>
          ...
          {resTr && (
            <ResBox>
              <div style={{ fontSize:12, color:COLOR, fontWeight:700, marginBottom:14 }}>RESULTADO — TRANSFORMADOR (IEC 60076 / IEEE C57)</div>
