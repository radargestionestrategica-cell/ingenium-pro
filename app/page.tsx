...
function calcMAOP(OD:number, t:number, SMYS:number, F=0.72, E_joint=1.0, T_op=20) {
  ...
  return {
    ...
    reg:ratio>0.15?'PARED GRUESA  Lame':ratio>0.10?'TRANSICIN':'PARED DELGADA  Barlow',
...
function calcDW(Q:number, D:number, L:number, rough=0.046, K_minor=0) {
  ...
  return {
    ...
    reg:Re<2300?'Laminar':Re<4000?'Transicion':'Turbulento'
  };
}
...
function interpretQuery(q: string) {
  ...
  if (lower.includes('maop')||lower.includes('presion maxima')||lower.includes('gasoducto')||lower.includes('oleoducto')) {
    ...
    if(!res) return null;
    return {
      type:'MAOP', title:'Presion Maxima Admisible (MAOP)', norma:'ASME B31.8 841.11',
      ...
      results:[
        ...
        {label:'Regimen', value:res.reg},
        {label:'Relacion t/D', value:`${res.ratio}%`},
      ], risk:'LOW' as RL
    };
  }
  if (lower.includes('golpe')||lower.includes('ariete')||lower.includes('joukowsky')) {
    ...
    return {
      ...
      results:[
        ...
        {label:'Sobrepresion', value:`${res.dP_MPa} MPa`, highlight:true},
        {label:'Sobrepresion', value:`${res.dP_bar} bar`},
        {label:'Tiempo critico', value:`${res.Tc} s`},
      ], risk:res.risk
    };
  }
  if (lower.includes('darcy')||lower.includes('perdida')||lower.includes('caudal')||lower.includes('hidraulica')||lower.includes('tuberia')) {
    ...
    if(!res) return null;
    return {
      type:'DARCY', title:'Perdidas Hidraulicas (Darcy-Weisbach)', norma:'Darcy-Weisbach / Colebrook-White',
      inputs:{Q:`${Q} L/s`, D:`${D*1000} mm`, L:`${L} m`},
      results:[
        {label:'Velocidad', value:`${res.V} m/s`, highlight:true},
        {label:'Perdida de carga', value:`${res.hf} m`, highlight:true},
  ...
  if (lower.includes('cimentacion')||lower.includes('portante')||lower.includes('meyerhof')||lower.includes('suelo')||lower.includes('fundacion')) {
    ...
    return {
      ...
      results:[
        {label:'Cap. ultima', value:`${res.qu} kPa`, highlight:true},
  ...
  if (lower.includes('dilatacion')||lower.includes('termica')||lower.includes('expansion')||lower.includes('temperatura')) {
    ...
    if(!res) return null;
    return {
      type:'THERMAL', title:'Dilatacion Termica (ASME B31.3)', norma:'ASME B31.3 Appendix C',
      inputs:{L:`${L} m`, T1:`${T1}C`, T2:`${T2}C`},
      results:[
        {label:'Dilatacion', value:`${res.dL} mm`, highlight:true},
        {label:'T', value:`${res.dT}C`},
        {label:'', value:`${res.alpha} 10/C`},
      ], risk:res.risk
    };
  }
  return null;
...
const riskLabel:Record<RL,string>={LOW:' SEGURO',MEDIUM:' MODERADO',HIGH:' ALTO',CRITICAL:' CRTICO'};
...
export default function IngeniumPro() {
 ...
 const [moduloActivo, setModuloActivo] = useState('petr?leo');
    ...
    if(calc){
      ...
      try{
        const res=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},
          body:JSON.stringify({message:`Eres INGENIUM PRO. El usuario consulto: "${userMsg}". Resultados de ${calc.title}: ${JSON.stringify(calc.results)}. Norma: ${calc.norma}. Analiza en 3 oraciones tecnicas profesionales en espanol: interpreta los resultados, si son seguros y que recomendar.`})});
        const data=await res.json();
        setMessages(prev=>prev.map(m=>m.loading?{...m,content:data.reply||'Analisis completado.',loading:false}:m));
      }catch{
        setMessages(prev=>prev.map(m=>m.loading?{...m,content:'Calculo completado segun normativa vigente.',loading:false}:m));
      }
    }else{
      try{
  ...
  const examples=['MAOP gasoducto 12" X65 junta ERW post-1970','Golpe de ariete acueducto DN400 L=2km','Perdidas hidraulicas Q=80 L/s D=300mm L=500m','Estabilidad talud 30 H=8m arcilla','Capacidad portante cimentacion B=2m Df=1.5m','Dilatacion termica 100m acero T=60C'];
    ...
    <div className="min-h-screen bg-[#0a0f1a] text-white flex flex-col" style={{fontFamily:'Inter,system-ui,sans-serif'}}>
        ...
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-slate-400">v8.0  Plataforma de Ingenieria Tecnica</div>
            </div>
          </div>
      ...
      <div className="border-b border-white/5 bg-[#0d1526]/50">
        <div className="max-w-5xl mx-auto px-4 py-2 flex gap-2 overflow-x-auto">
          {[{i:'petroleo',l:'Petroleo'},{i:'hidraulica',l:'Hidraulica'},{i:'mineria',l:'Mineria'},{i:'civil',l:'Civil'},{i:'geotecnia',l:'Geotecnia'},{i:'termica',l:'Termica'},{i:'vialidad',l:'Vialidad'},{i:'arquitectura',l:'Arquitectura'}].map(m=>(
                    ...
                    {msg.calcResult&&(
                      <div className="bg-[#111827] border border-white/10 rounded-2xl overflow-hidden">
                        ...
                        <div className="px-4 py-3 border-b border-white/5">
                          <div className="text-xs text-slate-500 mb-2 uppercase tracking-wider">Parametros</div>
      ...
      <div className="border-t border-white/10 bg-[#0d1526]/90 backdrop-blur-sm sticky bottom-0">
        <div className="max-w-4xl mx-auto px-4 py-3">
          <div className="flex gap-3 items-end">
            <div className="flex-1 bg-white/5 border border-white/10 focus-within:border-violet-500/50 rounded-xl transition-all">
              <textarea value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send()}}}
                placeholder="Consulta tu calculo tecnico de ingenieria... (Enter para enviar)"
          ...
          <div className="text-center mt-2">
            <span className="text-xs text-slate-600">INGENIUM PRO v8.0  ASME  API  AWWA  USACE   Silvana Belen Colombo 2026</span>
          </div>
        </div>
      </div>
